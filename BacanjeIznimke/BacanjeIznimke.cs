...
namespace Vsite.CSharp.Iznimke
{
    ...
    class BacanjeIznimke
    {
        ...
        static void Main(string[] args)
        {
            ...
            catch (ArgumentOutOfRangeException e) //zelimo tocno odedeni tip iznimke - svjesni sto ocekujemo
            {
                Console.WriteLine(e.Message);
                Console.WriteLine(e.ParamName);
                Console.WriteLine(e.ActualValue); //vrijednost koja je uzrokovala exception
                Console.WriteLine(e.Source); //to je assembly, to je ime naseg projekta BacanjeIznimke, to je dll
                Console.WriteLine(e.TargetSite);
                Console.WriteLine(e.StackTrace);
            }

            //moramo odvojiti zadnju liniju koda jer ona izaziva drugu vratu execptiona
            try
            {
                IspišiFaktorjel(17); // trebalo bi baciti iznimku zbog preljeva!
            }
            catch (OverflowException e) //zelimo tocno odedeni tip iznimke - svjesni sto ocekujemo
            {
                Console.WriteLine(e.Message);
